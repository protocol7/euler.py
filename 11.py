...
nums = list(map(parse, input.split("\n")))

m = 0
for r in range(len(nums)):
    for c in range(len(nums[0])):
        if c < len(nums[0]) - 3:
            m = max(m, product([nums[r][c+x] for x in range(4)]))
        if r < len(nums) - 3:
            m = max(m, product([nums[r+x][c] for x in range(4)]))
        if c < len(nums[0]) - 3 and r < len(nums) - 3:
